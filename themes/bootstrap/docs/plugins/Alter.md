...
`./first/src/Plugin/Alter`. No need to implement any code in
`first.theme`:
...
Create a file at `./first/src/Plugin/Alter/PageAttachments.php` with the
following contents:
...
/**
 * @file
 * Contains \Drupal\first\Plugin\Alter\PageAttachments.
 */

namespace Drupal\first\Plugin\Alter;
...
class PageAttachments extends BootstrapPageAttachements {
  ...
  public function alter(&$attachments, &$context1 = NULL, &$context2 = NULL) {
    ...
    $attachments['#attached']['library'][] = 'first/my_library';
  }
